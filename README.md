...
> These are **in-sample** results over the full history with no walk-forward — treat the pair as a screening result, not a validated live edge. Across the 91-pair sweep only ~10 pairs were solidly profitable and solvent, so the edge is concentrated. Adding an equity-drawdown stop barely changes this pair (its drawdown is given-back unrealized profit, not a capped entry loss — see below), but across the whole universe it sharply reduces tail risk: a 15% stop cuts the pairs that end insolvent from **29 → 0**.

## Example Equity Curve — Same Pair with a 15% Stop-Loss (`LTCUSDT-UNIUSDT`)

Re-running with a **15% equity-drawdown stop** (`stop_loss_frac = 0.15`) illustrates an honest limitation of *this* stop mechanism: it **barely changes `LTCUSDT-UNIUSDT`** and, importantly, does **not** reduce the pair's max drawdown.

<p align="center">
  <img src="data/plots/EQUITY_LTCUSDT-UNIUSDT_1d_w300_STOP15.png" alt="LTCUSDT-UNIUSDT equity curve (OLS, window 300, gross 1.0x, 15% stop-loss)" width="760" />
</p>

| Return | Sharpe (ann., 365) | Final equity | Max drawdown | Min equity | Trades (fills) |
|:------:|:------------------:|:------------:|:------------:|:----------:|:--------------:|
| **+123%** | **0.67** | $2,233 (from $1,000) | −46% | $954 | 29 |

> **Why the drawdown is essentially unchanged (−45% no-stop → −46% here).** `stop_loss_frac` caps a position's loss *relative to the equity at entry*. `LTCUSDT-UNIUSDT`'s drawdown is **given-back unrealized profit** — the spread runs to a large open gain, then reverts toward the entry level — so the loss *from entry* never reaches 15% and the stop never fires during the drawdown. A tighter stop here only trims realized upside (return +141% → +123%, Sharpe 0.72 → 0.67) without cutting the drawdown. Reducing *this* kind of drawdown would require a trailing / peak-to-trough stop, which the engine does not currently implement.
>
> **Where the stop *does* earn its keep — portfolio-wide tail risk.** Across the full 91-pair sweep the 15% stop cuts pairs that end **insolvent from 29 → 0**, lifts the worst final equity from **−$9,845 → +$9**, and tames the blow-up pairs whose loss genuinely *is* driven from entry — e.g. `BNBUSDT-XRPUSDT`'s max drawdown collapses from **−951% → −96%**. So the per-entry stop is a **risk control for screening/trading many pairs**, not a drawdown fix for a single well-behaved pair.
